...
-- 正在导出表  pro_cfg_manager.group_labels 的数据：~4 rows (大约)
...
-- 正在导出表  pro_cfg_manager.group_machines 的数据：~8 rows (大约)
...
-- 正在导出表  pro_cfg_manager.group_priv 的数据：~127 rows (大约)
...
-- 正在导出表  pro_cfg_manager.idc 的数据：~6 rows (大约)
DELETE FROM `idc`;
...
-- 正在导出表  pro_cfg_manager.job_group 的数据：~3 rows (大约)
...
-- 正在导出表  pro_cfg_manager.job_machines 的数据：~8 rows (大约)
...
-- 正在导出表  pro_cfg_manager.line 的数据：~6 rows (大约)
DELETE FROM `line`;
...
-- 正在导出表  pro_cfg_manager.machines 的数据：~771 rows (大约)
...
INSERT INTO `manager_group` (`id`, `name`, `enabled`, `update_at`, `update_by`) VALUES
	(1, 'administrator', 1, '2022-06-11 18:36:22', '');
...
INSERT INTO `manager_user` (`id`, `username`, `nice_name`, `password`, `phone`, `salt`, `group_id`, `update_at`, `enabled`, `create_at`, `update_by`) VALUES
	(1, 'admin', '管理员', '4e6b7093ddd49472d8157cab3da2177e', '10086', 'c9b39afa-61e1-42a9-a969-590832b79e9f', 1, '2022-06-11 18:36:22', 1, '2022-06-11 18:36:22', '');
...
-- 正在导出表  pro_cfg_manager.number_options 的数据：~0 rows (大约)
...
CREATE TABLE IF NOT EXISTS `operation_log` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8mb3 COMMENT='操作日志';
...
INSERT INTO `operation_log` (`id`, `username`, `operate_type`, `ipaddr`, `operate_content`, `operate_result`, `operate_at`, `operate_error`) VALUES
	...
	(8, 'admin', '', '127.0.0.1:59687', 'reset prometheus config data', 1, '2022-06-11 18:35:50', '成功'),
	(9, 'admin', '', '127.0.0.1:59848', 'reset prometheus config data', 1, '2022-06-11 18:38:08', '成功');
...
CREATE TABLE IF NOT EXISTS `page_function` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=135 DEFAULT CHARSET=utf8mb3 COMMENT='页面功能';

-- 正在导出表  pro_cfg_manager.page_function 的数据：~100 rows (大约)
...
INSERT INTO `page_function` (`id`, `page_name`, `page_nice_name`, `sub_page_name`, `sub_page_nice_name`, `func_name`, `func_nice_name`) VALUES
	...
	(133, 'jobs', 'JOB组管理', 'jobs', 'JOB组管理', 'get_job_label', '获取组标签'),
	(134, 'jobs', 'JOB组管理', 'jobs', 'JOB组管理', 'update_job_label', '更新组标签');
...
-- 正在导出表  pro_cfg_manager.pool 的数据：~7 rows (大约)
DELETE FROM `pool`;
...
) ENGINE=InnoDB AUTO_INCREMENT=60 DEFAULT CHARSET=utf8mb3 COMMENT='重写标签的配置';

-- 正在导出表  pro_cfg_manager.relabels 的数据：~18 rows (大约)
...
) ENGINE=InnoDB AUTO_INCREMENT=149 DEFAULT CHARSET=utf8mb3 COMMENT='用户登录会话';

-- 正在导出表  pro_cfg_manager.session 的数据：~0 rows (大约)
DELETE FROM `session`;
INSERT INTO `session` (`id`, `token`, `ipaddr`, `user_id`, `update_at`, `login_at`) VALUES
	(136, 'c2ffb331-29c9-4976-8d37-faa06381873f', '127.0.0.1:60861', 1, '2022-06-11 18:38:08', '2022-05-08 13:24:33');
...
-- 正在导出表  pro_cfg_manager.system_log 的数据：~193 rows (大约)
...
INSERT INTO `tmpl_fields` (`id`, `key`, `value`, `enabled`, `update_at`, `update_by`) VALUES
	(1, 'metrics', '/metrics', 1, '2022-06-11 18:38:08', ''),
	(2, 'refresh_interval', '15s', 1, '2022-06-11 18:38:08', '');
