<template>
  <div class="main-board">
    ...
    <el-table size="mini" highlight-current-row border :data="jobs" stripe :row-style="rowStyle" :cell-style="cellStyle">
        ...
        <el-table-column label="排序号" prop="display_order">
        </el-table-column>
        <el-table-column label="调整排序号">
            <template v-slot="scope">
            <div class="change_order_button">
            <el-button size="mini" type="primary" icon="el-icon-top" @click="doUp(scope)" plain></el-button>
            <el-button size="mini" type="primary" icon="el-icon-bottom" @click="doDown(scope)" plain></el-button>
            </div>
            </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center">
            <template v-slot="scope" align="center">
                <div class="actioneara">
                    ...
                    <div>
                        <el-popover trigger="click" :ref="`popover-${scope.$index}`" placement="top">
                        <p>确定删除吗？</p>
    ...
    <el-dialog
      title="新增IP地址" v-model="dialogVisible" width="400px" modal :before-close="handleClose">
      <span>
        <el-form label-position="right" :rules="rules" ref="addJobInfo" :model="addJobInfo" label-width="90px" size="small">
