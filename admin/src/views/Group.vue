<template>
  <div class="group-board">
      ...
      <div>
        ...
        <div class="table-show">
          <el-scrollbar height="75vh" class="flex-content">
            <el-table size="mini" highlight-current-row border :data="ManagerGroup" stripe :row-style="rowStyle"
              ...
              <el-table-column label="用户数" prop="user_count" width="150px">
                <template v-slot="{ row }">
                  <el-button size="mini" @click="routeToUser(row)" type="text">{{ row.user_count }}</el-button>
                </template>
              </el-table-column>
              <el-table-column label="更新账号" align="center" prop="update_by" show-overflow-tooltip></el-table-column>
              <el-table-column label="更新时间" width="150px" prop="update_at">
                <template v-slot="{ row }">
                  ...
                  <div class="actioneara">
                    <div>
                      <el-button size="mini" type="primary" @click="doEdit(scope)" plain
                        :disabled="editCodeButVisable[scope.row.id]">编辑</el-button>
                    </div>
                    <div>
                      <el-button size="mini" type="primary" @click="doUserList(scope)" plain>用户列表</el-button>
                    </div>
                    <div>
                    ...
                    <div>
                      <el-button size="mini" type="primary" @click="doPriv(scope)" plain>权限</el-button>
                    </div>
                    <div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="400px" modal :before-close="handleClose">
      <span>
        <el-form label-position="right" :rules="rules" ref="ManagerGroupRef" :model="ManagerGroupRef" label-width="auto"
          size="small">
          <el-form-item label="组名：" prop="name">
            <el-input style="width: 280px" v-model="ManagerGroupRef.name"></el-input>
          </el-form-item>
          <el-form-item size="small">
            <el-button size="small" type="primary" @click="onSubmit('ManagerGroupRef')">{{ buttonTitle }}</el-button>
            <el-button size="small" type="info" @click="onCancel('ManagerGroupRef')">取消</el-button>
          </el-form-item>
        </el-form>
      </span>
    </el-dialog>
    <el-dialog :title="'编辑组成员：' + editObject" v-model="editUserVisible" width="700px" modal
      :before-close="editUserClose">
      <el-row type="flex" align="middle" justify="center">
        <el-transfer v-model="usersListValue" filterable :filter-method="filterUserMethod" filter-placeholder="请输入关键字"
          :data="usersListData" @change="transferChange" :titles="['用户列表', '组成员']">
        </el-transfer>
      </el-row>
      <div class="user-list-push-box">
        <el-button class="user-list-close-btn" type="info" size="small" @click="editUserClose">关闭</el-button>
        <el-button class="user-list-push-btn" type="warning" size="small" @click="pushNewUserList">提交</el-button>
      </div>
    </el-dialog>
  </div>
</template>
